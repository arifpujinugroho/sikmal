        ...
        <div class="row align-items-end">
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        <h4>Cari Jodoh Anggota</h4>
                        <span class="text-info">Daftar Mahasiswa <strong>niat</strong> <del class="text-muted">nikah</del> <strong>ikut PKM</strong> tapi belum punya <del class="text-muted">Mahar</del> <strong>Ide & Kelompok</strong></span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                                        ...
                                        @if ($jmlall != 0 || $jmlme != 0)
                                            @if($jmlme != 0)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{$m->nim}}</td>
                                                    <td>{{$m->nama_prodi}}</td>
                                                    <td>{{$m->cardoh_skill}}</td>
                                                    <td>
                                                        <div class="btn-group">
                                                            <button data-kode="{{Crypt::encryptString($m->id_cardoh)}}" data-nama="{{$m->nama}}" data-kebutuhan="{{$m->cardoh_skill}}" class="editCardoh btn btn-mini btn-warning" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></button>
...
<div class="modal fade" id="tambah-cardoh" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{url('mhs/cardoh/tambahkelompok')}}" method="post">
            @csrf
                <div class="modal-body">
                    <div class="form-group">
                      <label>Kemampuan/Kelebihan</label>
                      <input type="text"
                        class="form-control" name="kebutuhan" placeholder="Jabarkan kemampuan/kelebihan" required>
                    </div>
                </div>
...
<div class="modal fade" id="edit-cardoh" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Cari Jodoh</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
            </div>
            <form action="{{url('mhs/cardoh/editkelompok')}}" method="post">
            @csrf
            <input name="kode" type="hidden" id="kode">
                <div class="modal-body">
                    <div class="form-group">
                      <label>Kemampuan/Kelebihan</label>
                      <input type="text"
                        class="form-control" name="kebutuhan" id="kebutuhan" placeholder="Jabarkan kemampuan/kelebihan" required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-sm btn-success">Save</button>
