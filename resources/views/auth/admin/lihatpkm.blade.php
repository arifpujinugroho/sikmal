...
@section('title')
{{$pkm->skim_singkat}} {{$ketuapkm->nama}}
@endsection
...
@if ($jumlahanggota < 2) <div class="alert alert-danger">
    @if($pkm->status_upload == 1 && $pkm->aktif == 1)
    Belum ada Anggota
    @else
								...
								<div class="col-md-12 mb-3">
									...
									<div class="row">
										@if($pkm->file_proposal != "" || $pkm->file_proposal != NULL)
											<div class="col-md-2">
												<h6><a href="{{url('admin/downpro')}}/{{Crypt::encryptString($id)}}"><button class="btn btn-mini btn-success">Download Proposal</button></a></h6>
											</div>
										@endif
    ...
    {{--Ini End--}}
    	    ...
    	    <div class="modal fade" id="tambah-anggota" tabindex="-1" role="dialog" aria-labelledby="tambahAnggotaLabel" aria-hidden="true">
    	    	<div class="modal-dialog">
    	    		<div class="modal-content">
    	    			...
    	    			<form action="{{url('admin/tambahanggota')}}" class="form-validation" method="POST">
    	    				@csrf
    	...
    	<div class="modal fade" id="hapus-pkm" tabindex="-1" role="dialog" aria-labelledby="hapusPKMLabel" aria-hidden="true">
    		<div class="modal-dialog">
    			<div class="modal-content">
    				<form action="{{url('admin/hapuspkm')}}" method="post">
    					@csrf
    					<div class="modal-header">
    						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
    	...
    	<div class="modal fade" id="hapus-anggota" tabindex="-1" role="dialog" aria-labelledby="hapusAnggotaLabel" aria-hidden="true">
    		<div class="modal-dialog">
    			<div class="modal-content">
    				<form action="{{url('admin/hapusanggota')}}" method="post">
    					@csrf
    					<div class="modal-header">
    						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	    ...
	    @if($pkm->file_proposal == "")
	        <div class="modal fade" id="upload-proposal" tabindex="-1" role="dialog" aria-labelledby="uploadProposalLabel" aria-hidden="true">
	        	<div class="modal-dialog">
	        		<div class="modal-content">
	        			<form action="{{url('admin/uploadproposalcustom')}}" class="form-validation" method="post" enctype="multipart/form-data">
	        				@csrf
	        				<div class="modal-header">
	        					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	    ...
	    @if($pkm->file_proposal != "")
	        <div class="modal fade" id="revisi-proposal" tabindex="-1" role="dialog" aria-labelledby="revisiProposalLabel" aria-hidden="true">
	        	<div class="modal-dialog">
	        		<div class="modal-content">
	        			<form action="{{url('admin/revisiproposal')}}" class="form-validation" method="post" enctype="multipart/form-data">
	        				@csrf
	        				<div class="modal-header">
	        					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	    ...
	    <div class="modal fade" id="edit-pkm" tabindex="-1" role="dialog" aria-labelledby="editPKMLabel" aria-hidden="true">
	    	<div class="modal-dialog">
	    		<div class="modal-content">
	    			<form action="{{url('admin/editpkm')}}" class="form-validation" method="post">
	    				@csrf
	    				<div class="modal-header">
	    					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
	    				...
	    				<div class="modal-body">
							<div class="form-group">
	    						...
	    						<div class="input-group">
	    							...
	    							<select class="validate[required] form-control" name="tahun">
	    								<option hidden value="{{$pkm->id_tahun_pkm}}">{{$pkm->tahun}}</option>
	    								@foreach($tahun as $t)
							...
							<div class="form-group">
	    						...
	    						<div class="input-group">
	    							<span class="input-group-addon"><i class="fa fa fa-calendar fa-fw"></i></span>
	    							<select class="validate[required] form-control" name="skim">
	    								<option hidden value="{{$pkm->id_skim_pkm}}">{{$pkm->skim_lengkap}}</option>
	    								@foreach($skim as $t)
	    								<option value="{{$t->id }}">{{ $t->skim_lengkap }}</option>
	    								@endforeach
	    							</select>
	    						</div>
	    					</div>
...
@section('footer')
    ...
    <script>
		...
		$('#id_fakultas').on('change', function() {
			...
			$.get("{{ URL::to('/admin/dosen-fakultas') }}", {
					id_fakultas: id_fakultas,
					tipepkm: tipepkm
				})
				.done(function(result) {
